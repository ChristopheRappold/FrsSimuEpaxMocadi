...
void SIS_Beam(std::string name_in,double EkI=2.,double BrhoI = 18.,int Zmax = 15,bool stable=true)
{
  ...
  while(std::getline(ifs,temp_line))
	...
	continue;
      ...
      if(Z>Zmax)
	continue;
      if(table_stable[A][Z]==false && stable==true)
	continue;
      if(stable==false && Q!=Z)
	continue;
      ...
      if(Brho<=BrhoI)
	{
	  h_Brho->Fill(name_temp,Brho);
	}


      double params[3] = {-1.*A*A,-A*Mass,(BrhoI*Q/3.10715497)*(BrhoI*Q/3.10715497)};
