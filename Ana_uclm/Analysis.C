	  ...
	  if(TMath::Abs(max_G-maxMax)<=1e-4)
	    {
	      ...
	      dataMax tempMax = {i,j,alpha,beta, nameTemplateB, nameTemplateT, max_CmTarget, maxIntensity, maxContamination};
...
void Invariant(std::vector<double>& In, const std::vector<double*>& Range, int type, TH2F* h_hist)
{
...
int AnalysisHyp(std::string namefileEpax, std::string namefileHyp, std::string Hyp, int type = 0, std::string ICtype = "I-C", std::string Mtype = "A B AB", int Ntype = -1, int NtypeComp = 1, double minHist = 0)
{
  ...
  while(std::getline(ifs,temp_line))
    {
      ...
      if(it_FBT==BeamTarget.end())
	{
	  tuple_graph temp(SnameBT,SnameF,idsBT,CmTarget,gCmTarget,PrimaryBeamInt*prod1*(1.-SurvivalRate),PrimaryBeamInt*prod2*(1.-SurvivalRate),PrimaryBeamInt*prod3*(1.-SurvivalRate),PrimaryBeamInt*prod4*(1.-SurvivalRate),MeanEnergy,SigmaEnergy,SurvivalRate);
	  BeamTarget.insert(std::pair<std::string,tuple_graph>(SnameAll,temp));
	}
      else
	{
	  it_FBT->second.AddValue(CmTarget,gCmTarget,PrimaryBeamInt*prod1*(1.-SurvivalRate),PrimaryBeamInt*prod2*(1.-SurvivalRate),PrimaryBeamInt*prod3*(1.-SurvivalRate),PrimaryBeamInt*prod4*(1.-SurvivalRate),MeanEnergy,SigmaEnergy,SurvivalRate);
	}
  ...
  for(int i=1;i<=h_hist->GetNbinsX();++i)
	...
	//for(int k=1;k<=h_hist->GetNbinsZ();++k)
	{
	  ...
	  double max_G = -1.e100;
	  int max_CmTarget = -1;
	  std::string max_namePrimary;
	  std::string max_nameSecondary;
	  ...
	  for(auto it_secondaryReaction = hyp_prod.cbegin(), it_secondaryReactionEnd = hyp_prod.cend(); it_secondaryReaction != it_secondaryReactionEnd; ++it_secondaryReaction)
		...
		if(it_secondaryReaction->nameB == it_FBT->second.F)
		  {
		    std::string tempNameP (it_FBT->second.BT);
		    std::vector<int> tempIds(it_FBT->second.BT_ids);
			...
			std::vector<double> Input = {tempT,tempI,tempC};
			Invariant(Input, RangeAll[type], type, h_Internal);
			...
			double tempG =  - outPar[0]*Input[0] + outPar[1]*Input[1] + outPar[2]*Input[2] ;
			...
			if(tempG > max_G)
			  {
			    ...
			    maxCrossSection = it_FBT->second.ProdFrag[idCmTarget]*it_secondaryReaction->cross_section;
			  }
		      }
		  }
  ...
  for(unsigned int maxIndex = 0; maxIndex < maxMaxId.size(); ++maxIndex)
    {
      std::cout<<" Max ["<<maxIndex<<"] -> ("<<maxMaxId[maxIndex].idI<<", "<<maxMaxId[maxIndex].idJ<<") | alpha="<<maxMaxId[maxIndex].alpha<<" beta="<<maxMaxId[maxIndex].beta<<" "<<std::endl;
      std::cout<<" +-> beamP:"<<maxMaxId[maxIndex].beam<<" target:"<<maxMaxId[maxIndex].target<<" IDcm:"<<maxMaxId[maxIndex].cmtarget<<" Int:"<<maxMaxId[maxIndex].intensity<<" | cont:"<<maxMaxId[maxIndex].contamination<<std::endl;
      std::cout<<" +-> beamS:"<<maxMaxId[maxIndex].fragment<<" Energy:"<<maxMaxId[maxIndex].energyMean<<" +- "<<maxMaxId[maxIndex].energySigma<<" Survival:"<<maxMaxId[maxIndex].survivalRate<<std::endl;
      std::cout<<" +-> CX:"<<maxMaxId[maxIndex].crossSectionHyp<<std::endl;
      maxMarkers[maxIndex] = new TMarker(maxMaxId[maxIndex].alpha,maxMaxId[maxIndex].beta,30);
    }
